  ...
  indent: function () {
    ...
    if (scopes['heading'] && scopes['symbol']) {
      ...
      editor.transact(0, () => {
        ...
        if (scopes.chat) this.rotLabel(SymbolConf.chat)
      })
    }
    else {
  ...
  newLine: function () {
    ...
    editor.transact(0, () => {
      if ((curScopes.includes('bullet') || curScopes.includes('quote') || curScopes.includes('heading')) && isCursorAtEndOfLine) {
        if (!lineScopes.text) {
        ...
        else if (lineScopes.chat && !lineScopes.chat_text) {
          editor.deleteToBeginningOfWord()
        }
        else {
          ...
          if (lineScopes.chat_text) {
            editor.insertText(lineScopes.chat_symbol.text + ' ')
          }
        }
      }
      else {
