...
const BufferUtils = {
  ...
  createList: function () {
    this.createElement('bullet', SymbolConf.bullet)
  },
  ...
  indent: function () {
    ...
    if (scopes['heading'] && scopes['symbol']) {
      ...
      var symbol = SymbolConf.bullet // default
      switch (level) {
...
function toggleEmphasis (type) {
  ...
  var text = editor.getWordUnderCursor()
  if (!text || text==symbol) return  
  ...
  if (curScopes.indexOf('emphasis.' + type) > -1) { 
    ...
    while (1) {
      ...
      if (text.includes(symbol)) { 
        rangeStart =  editor.getSelectedBufferRange()
        break;
      }  
    }
    ...
    while (1) {
      ...
      text = editor.getSelectedText()
      if (text.includes(symbol)) { 
        rangeEnd =  editor.getSelectedBufferRange()
        break;
      }  
    }
    ...
    var re = new RegExp(replaceRegExp, 'g');
    ...
    // check if we have any range selected
