...
const BufferUtils = {
  ...
  toggleElement: function (scopeName, symbol, addTab) {
  ...
  createHeading1: function () {
    this.toggleElement('h1', SymbolConf.heading)
  },

  createHeading2: function () {
    this.toggleElement('h2', SymbolConf.heading2)
  },

  createHeading3: function () {
    this.toggleElement('h3', SymbolConf.heading3)
  },

  createList: function () {
    this.toggleElement('bullet', SymbolConf.list)
  },

  createQuote: function () {
    this.toggleElement('quote', SymbolConf.quote)
  },
  ...
  indent: function () {
    ...
    if (scopes['heading'] && scopes['symbol']) {
      ...
      var symbol = SymbolConf.list; // default
      // switch (level) {
  ...
  outdent: function () {
    ...
    if (scopes['heading']) {
      ...
      if (level >= 0) {
        ...
        this.toggleElement('bullet', SymbolConf.list)
        // if (!scopes['text']) editor.deleteToBeginningOfSubword()    
      }
    }
    else {
      editor.outdentSelectedRows()
  ...
  insertBlankLine: function (where) {
    editor = atom.workspace.getActiveTextEditor()
    editor.transact(0, () => {
      range = editor.getBuffer().rangeForRow(editor.getCursorBufferPosition().row)
      curText = editor.getTextInBufferRange(range)
      p1 = editor.getCursorBufferPosition()
      
      if (where == 'before') {
        editor.setTextInBufferRange(range, '\n' + curText)
      }
      else {
        editor.setTextInBufferRange(range, curText + '\n')
      }
      editor.setCursorBufferPosition(p1)
    })
  },
  ...
  newLine: function () {
    ...
    editor.transact(0, () => {
      if ((curScopes.includes('bullet') || curScopes.includes('bullet-inner') || curScopes.includes('quote')) && isCursorAtEndOfLine) {
        if (!lineScopes.text) {
