...
const BufferUtils = {
  ...
  indent: function () {
    ...
    if (scopes['heading'] || scopes['justtext']) {
      ...
      editor.transact(0, () => {
        if (symbol=='*' && leadingSpace>=BulletConf.tabsize) this.rotLabel(BulletConf.list)
        editor.indentSelectedRows()
      })
    }
    else editor.transact(0, () => {
      this.rotLabel(BulletConf.list) // add a bullet
      editor.indentSelectedRows()
    })
  },
  ...
  outdent: function () {
    ...
    if (scopes['heading']) {
      ...
      var leadingSpace = scopes['leadingspace'] ? scopes['leadingspace'].text.length : 0
      editor.transact(0, () => {
        if (symbol=='-' && leadingSpace<=BulletConf.tabsize * 2) this.rotLabel(BulletConf.list)
