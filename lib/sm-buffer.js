...
const BufferUtils = {
  ...
  indent: function () {
    ...
    if (scopes['headerline']) this.rotLabel(BulletConf.heading, 1)
  ...
  outdent: function () {
    ...
    const scopes = getSelectedLineScopes(editor)
    if (scopes['headerline']) this.rotLabel(BulletConf.heading, -1)
    else editor.outdentSelectedRows()
  },
  ...
  star: function () {
    editor = atom.workspace.getActiveTextEditor()
    const scopes = getSelectedLineScopes(editor)
    if (scopes['bullet']) this.rotLabel(BulletConf.star)
    else this.rotLabel(BulletConf.bullet, 1)
  },
  ...
  newBulletLine: function () {
    ...
    if (curScopes.includes('bullet') && curScopes.includes('text')) {
