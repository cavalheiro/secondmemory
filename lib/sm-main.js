  ...
  activate: function (state) {
    ...
    atom.workspace.observeTextEditors((editor) => {
      ...
      if (editor.getGrammar().scopeName == 'source.secondmemory') {
        subscriptions.add(editor.onDidSave( (f) => {
          // avoid clashing of events when removing window focus with autosave enabled
          if (reindex[f.path]) return
          reindex[f.path] = true
          setTimeout(() => {
            smIndex.reIndexFile(f.path)
            delete reindex[f.path]
          }, 100)
        }))
      }
    })
    ...
    subscriptions.add(
      ...
      atom.commands.add('atom-text-editor', { 'sm:unindent': (event) => BufferUtils.outdent() }),
